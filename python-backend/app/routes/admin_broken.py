...
class SecureDoctorRegistrationRequest(BaseModel):
    ...
    phone: str = Field(..., regex=r"^\+?1?[0-9]{10,15}$")
    
    # Professional Identity
    medical_license: str = Field(..., min_length=8, max_length=50)
    dea_number: Optional[str] = Field(None, regex=r"^[A-Z]{2}\d{7}$")
    npi_number: str = Field(..., regex=r"^\d{10}$")
    ...
    telemedicine_states: List[str] = Field(..., min_items=1)
